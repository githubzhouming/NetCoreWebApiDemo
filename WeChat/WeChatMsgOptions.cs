...
namespace ZM.WeChat
{
    public class WeChatMsgOptions 
    {
        ...
        {
            var result = "";
            var baseUrl = "http://www.zmserver.cn";//
                ...
                case WeChatMessageType.Event:
                    ...
                    switch (eventType)
                    {
                        case "subscribe"://用户未关注时，进行关注后的事件推送
                            ...
                            //var WeChatInfo = UserAdminAPI.GetInfo(token, openId);//注意：订阅号没有此权限
                            ...
                            if (!string.IsNullOrEmpty(eventKey))
                            {
                                ...
                                result = ReplayPassiveMessageAPI.RepayNews(openId, myUserName,
                                    new WeChatNews
                                    {
                                        ...
                                        picurl = string.Format("{0}/ad.jpg", baseUrl),
                                        url = baseUrl
                                    });
                            }
                            else
                        ...
                        case "image"://图片消息
                            ...
                            result = ReplayPassiveMessageAPI.RepayNews(openId, myUserName, new WeChatNews
                            {
                                title = "您刚才发送了图片消息",
                                picurl = string.Format("{0}/Images/ad.jpg", baseUrl),
                                description = "点击查看图片",
                                url = userImage
                            });
                            #endregion
                            ...
                            #region 自定义菜单事件
                            {
                                switch (eventKey)
                                {
                                    case "myaccount"://CLICK类型事件举例
                                        #region 我的账户
                                        result = ReplayPassiveMessageAPI.RepayNews(openId, myUserName, new List<WeChatNews>()
                                    {
                                        new WeChatNews{
                                            title="我的帐户",
                                            url=string.Format("{0}/user?openId={1}",baseUrl,openId),
                                        ...
                                        #endregion
                                        break;
                                    case "www.WeChatsdk.net"://VIEW类型事件举例，注意：点击菜单弹出子菜单，不会产生上报。
                                        //TODO:后台处理逻辑
                                        break;
                                    default:
                        ...
                        case "pic_photo_or_album"://弹出拍照或者相册发图的事件推送
                            ...
                            break;
                        case "pic_WeChat"://弹出微信相册发图器的事件推送
                            {
                                var count = message.Body.SendPicsInfo.Count.Value;//发送的图片数量
                                var picList = message.Body.PicList.Value;//发送的图片信息
